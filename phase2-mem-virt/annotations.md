...
**EPT 页表级数与 Guest 物理地址位数（MAXPHYADDR）的关系**：

| 页表级数 | 适用场景 | 覆盖物理地址范围 | EPTP 配置 |
|---------|---------|-----------------|-----------|
| 4 级 | Guest MAXPHYADDR ≤ 48 | 48 位物理地址（256 TB） | `VMX_EPTP_PWL_4` |
| 5 级 | Guest MAXPHYADDR > 48 | 52 位物理地址（4 PB） | `VMX_EPTP_PWL_5` |

**注意**：
- 48 位和 57 位是**虚拟地址**的位数（Linear Address，由 LA57 控制）
- 但 EPT 页表级数由 Guest 的**物理地址位数**（MAXPHYADDR）决定
- EPT 页表中的物理地址字段是 bits 51:12，最多支持 52 位物理地址
