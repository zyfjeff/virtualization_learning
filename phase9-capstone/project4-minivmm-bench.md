...
**前置**：项目 1-3；阅读 `../phase10-performance/measurement.md`（测量纪律，**先读**）、
`../phase10-performance/parameters.md` §1（halt-polling 参数默认值与权限）、
`../phase0-kvm-framework/annotations.md` §9（halt-polling 机制）、
`../phase1-cpu-virt/README.md` §1（VPID 机制）、`../phase11-debugging/README.md`
...
- 把 exit 类型映射回源码（`kvm:kvm_exit` 的 reason 枚举与
  `../phase11-debugging/` 中的 tracepoint 目录），解释每类退出
  在你的实现里对应哪段代码
...
**默认值与权限的单一来源是 `../phase10-performance/parameters.md` §1，本表不抄数字。**
本实验只需要知道两件事：
...
- 生效的**上限**由 `kvm_vcpu_max_halt_poll_ns()`（`virt/kvm/kvm_main.c:3787-3802`）决定：
  ...
  先查 VMM 有没有设这个 cap（`../phase10-performance/measurement.md` §6 归因纪律）。
...
1. `echo 0 > /sys/module/kvm/parameters/halt_poll_ns` 与默认值对比：
   ...
   ★ 写之前先记下原值、`trap ... EXIT` 恢复（`../phase10-performance/measurement.md`
...
2. 用 `kvm:kvm_halt_poll_ns` tracepoint 观察窗口增长/缩减轨迹
   ...
   采样与清场方法见 `../phase10-performance/measurement.md` §4、§5，
   工具用法见 `../phase11-debugging/README.md`）
...
- `../phase10-performance/measurement.md`（测量纪律与扰动预算）、
  `../phase10-performance/parameters.md`（参数默认值/权限唯一来源）、
  `../phase10-performance/index.md`（本仓已实测结论索引，含 halt-polling 收益曲线）、
  `../phase11-debugging/README.md`、`../phase11-microvm/README.md`
