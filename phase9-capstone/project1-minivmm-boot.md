...
`../examples/kvm-api-demo/kvm-demo.c` 与 `../phase8-timer-virt/practice/common.h`。
...
3. `KVM_CREATE_PIT`（case @ `x86.c:7125`）可选：guest 使用 TSC-deadline
   ...
   `../phase8-timer-virt/README.md`）。先不创建，跑不通再回头加。
...
2. `KVM_SET_CPUID2`（case @ `x86.c:5957`）。推荐直接以
   ...
   - **leaf1 ECX bit24 `X86_FEATURE_TSC_DEADLINE_TIMER` 必须保留**：
     ...
     `../phase8-timer-virt/practice/` 实验 3 实测踩过的坑（该 README
     §前置条件 ②，细节勘误见 `../phase8-timer-virt/corrections.md`）。
...
3. **x2APIC 的写不是"静默丢弃"**：若 CPUID 暴露 x2APIC，guest 用
   ...
   `../phase8-timer-virt/corrections.md`。
...
- 起步代码：`../examples/kvm-api-demo/kvm-demo.c`、
  `../phase8-timer-virt/practice/common.h`
