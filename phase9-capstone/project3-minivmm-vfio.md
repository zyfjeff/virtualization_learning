...
`../phase7-vfio/README.md`（VFIO 全栈：§1.4 group 划分、§1.5 ACS/ATS、
...
1. **确认 group 可直通**：按 `../phase7-vfio/README.md` §1.4 的方法
...
2. **解绑原驱动、绑定 `vfio-pci`**（`scripts/` 与
   `../phase7-vfio/practice/` 里有现成的认领/跟踪实验可复用）。
...
（完整序列见 `../phase7-vfio/README.md` 的架构总览一节。）
...
3. BAR：`VFIO_DEVICE_GET_REGION_INFO` 枚举各 BAR region；
   ...
   对照 `../phase7-vfio/README.md` 中 BAR mmap 与剥页的讨论。

## M4：中断（MSI/MSI-X）

复用 `../phase7-vfio/README.md` 已核实的流程（QEMU 始终在中间协调）：

1. guest 写 MSI-X 表 → VMM 在自己模拟的 BAR 里拦截（对照
   `../phase7-vfio/README.md` 中 `msix_table_mmio_write` 流程）
...
4. 用 `KVM_IRQFD`（case @ `kvm_main.c:5257`）把同一组 eventfd 绑到
   ...
   要求**，见 `../phase7-vfio/README.md` 的相应讨论）
5. 中断重映射（IRTE）由 VFIO 内核驱动在 `intel/irq_remapping.c`
   ...
   与 `../phase5-interrupts/` 的 PI 章节）。**先跑通 Remapped 模式
   再谈 Posted**。
...
- `../phase7-vfio/README.md` + `annotations.md` + `practice/`
