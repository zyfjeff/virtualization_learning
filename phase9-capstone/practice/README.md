...
4. **phase9 优化在本场景的可测性**：
   - **halt-polling**：可测（M3）。本场景的正确结论是"默认参数即可，
     ...
     收益曲线的实测数据在 `../../phase10-performance/index.md` §1.2（本仓规则：
     数字只放一处，这里只给指针），参数默认值/权限在
     `../../phase10-performance/parameters.md` §1。
   - **PLE**：单 vCPU、无超卖自旋，测不出；需要多 vCPU 超卖实验 ——
     那一套的设计与判据在 `../../phase10-performance/practice/bench-ple.md`（E1），
