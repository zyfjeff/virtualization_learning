...
**修正后的排查方式**：不要盯着 bind 是否成功，要看 `SET_CONTAINER` 的 errno。`phase6-vfio/practice/vfio-claim-trace.c` 在 `-EPERM` 时会直接给出提示。
...
- 相关章节：`phase4-interrupts/`（Posted Interrupts 与 VT-d IR 机制）
