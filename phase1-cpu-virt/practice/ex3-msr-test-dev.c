/*
 * 练习 3: MSR 访问测试 (使用 /dev/cpu/0/msr)
 *
 * 目标: 通过设备文件读取 MSR，测量访问时间
 */
...
// 通过设备文件读取 MSR
int read_msr_dev(int fd, unsigned int msr, unsigned long long *value) {
    off_t offset = msr;
...
int main() {
    ...
    if (msr_fd < 0) {
        ...
        printf("  提示: 确保内核配置了 CONFIG_X86_MSR=y\n");
        return 1;
    }
    ...
    for (int i = 0; i < ITERATIONS; i++) {
        read_msr_dev(msr_fd, 0x10, &value);
    }
    ...
    printf("   最后一次读取值: 0x%llx\n", value);
    printf("   说明: EFER 通常被 KVM 拦截，触发 VM-Exit\n\n");

    // 测试 3: IA32_APIC_BASE (0x1B) - 通常拦截
    printf("3. IA32_APIC_BASE (MSR 0x1B) - 通常拦截\n");
    clock_gettime(CLOCK_MONOTONIC, &start);
    for (int i = 0; i < ITERATIONS; i++) {
        read_msr_dev(msr_fd, 0x1B, &value);
    }
    clock_gettime(CLOCK_MONOTONIC, &end);
    ...
    printf("   最后一次读取值: 0x%llx\n", value);
    printf("   说明: APIC_BASE 通常被 KVM 拦截\n\n");
    ...
    printf("\n");
    printf("注意: 这里的开销包括:\n");
    printf("  1. read() 系统调用开销\n");
    printf("  2. 内核态 rdmsr 指令开销\n");
    printf("  3. 如果被拦截，还有 VM-Exit 开销\n");
    printf("\n");
