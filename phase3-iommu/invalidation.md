...
- **只想理解核心语义？** → 读 I.1 + I.2 + I.16（自检）
- **想深入 Intel 实现？** → 读 I.3–I.8（失效队列完整机制）
- **想看三家对比？** → 读 I.12–I.14（SMMUv3 / AMD-Vi / 横向对照）
- **想动手观测？** → 读 I.15 + practice/ex3（tracepoint + 延迟直方图）
- **关心 Device-TLB？** → 读 I.9（ATS 失效的代价）
- **想了解 strict/lazy？** → 读 I.11（DMA_FQ 的延迟策略）
