...
10. **`PCI_EXP_TYPE_PCI_BRIDGE` 与 `PCI_EXP_TYPE_PCIE_BRIDGE` 的名字与含义正好相反** — `PCI_EXP_TYPE_PCI_BRIDGE` = 0x7 = "PCIe to PCI/PCI-X Bridge"，`PCI_EXP_TYPE_PCIE_BRIDGE` = 0x8 = "PCI/PCI-X to PCIe Bridge"（`include/uapi/linux/pci_regs.h:482-483`）。`lspci` 印的 "PCI-Express to PCI/PCI-X Bridge" 是**前者**。别照名字猜，用 `setpci -s <BDF> <PCIe_Cap+2>.w` 读 `PCI_EXP_FLAGS` bits 7:4 确认。
11. **ACS 判定：`ACSCap` 里的 `-` 不算失败** — `pci_acs_flags_enabled()` 先按设备声明的 `ACSCap` 掩掉要求集（`drivers/pci/pci.c:3598`，`acs_flags &= (cap | PCI_ACS_EC)`），**没声明的能力被当作硬连线已启用**。只有「Cap 里有、Ctl 里没开」才算不通过。另外 `REQ_ACS_FLAGS` 只含 SV/RR/CR/UF（`drivers/iommu/iommu.c:1383`），`TransBlk-` / `EgressCtrl-` 与判定无关。
