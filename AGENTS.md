...
1. **IRTE 模式选择位与 DM/DLM** — 区分 Remapped / Posted 的是 `IM` (IRTE Mode) @ bit 15，两种格式都有该位：Remapped 是 `IM=0`，Posted 是 `IM=1`。**不要写成 `DM=0`/`DM=1`**。Remapped 格式里 `DM` (Destination Mode) @ bit 2 与 `DLM` (Delivery Mode) @ bits 7:5 是两个不同字段，别把 DLM 叫成 DM。（VT-d Spec 9.9 / Figure 9-9 与 9.10 / Figure 9-10）
