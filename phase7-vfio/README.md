...
>
> **前置依赖**:
> - 第五阶段（中断虚拟化 + VT-d IR）：直通设备的中断投递依赖 PI + IRTE 机制
> - 第六阶段（Virtio）：理解半虚拟化 I/O 与直通的区别和取舍
...
> **详见 phase5-interrupts/posted-interrupts.md §2** 的 IRTE 格式详解和 SVT/SID/SQ 字段定义。
...
这是 phase4（中断虚拟化）与 phase6 真正接壤的地方。要点是**职责分工**：
...
#   **全部**已启用事件清掉，只想加事件一律 `>>`
#   （../phase10-performance/measurement.md §5 第 3 条）。
