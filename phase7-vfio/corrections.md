...
**修正后的排查方式**：不要盯着 bind 是否成功，要看 `SET_CONTAINER` 的 errno。`phase7-vfio/practice/vfio-claim-trace.c` 在 `-EPERM` 时会直接给出提示。
...
发现时机：2026-09-03 做 `phase10-performance` 重设计时顺带核对（机制登记在
`../phase10-performance/corrections.md` D10 / D11 / D12），三组都已改。
...
`../phase10-performance/measurement.md` §5 第 3 条：显式清场 + 一个名字一次 `>>`，
...
1195 行）—— 本仓 `../phase9-capstone/practice/mini-kvm/check-refs.py` 正是这么报的。
...
发现时机：2026-09-03，跑 `../phase9-capstone/practice/mini-kvm/check-refs.py` 做本轮验收。
...
- 实验程序：`phase7-vfio/practice/`（说明见该目录 `README.md`）
- 相关章节：`phase5-interrupts/`（Posted Interrupts 与 VT-d IR 机制）
