...
这条性质把 [Q2](group.md) 里 group 与 phase5 的接缝缝死了：**ACS 配置通过改变 group 的
成员，间接改变了域的形状**（D.7 展开）。
...
| group 成员 | `ls /sys/kernel/iommu_groups/N/devices/` | 见 phase5 §1.4 的实测方法 |
...
| 是否真的走了 IOMMU | 设备 `dma_map` 后读 `iommu_groups/*/devices` + `type`，或直接看失效计数 | sysfs 不反映 VFIO 接管，见 [phase5 corrections](../phase5-vfio/corrections.md) |
...
[phase5](../phase5-vfio/README.md) 里那套看起来"只是安全配置"的 ACS 讲清楚：

```
ACS 能力位/拓扑  ──(phase5 §1.4/§1.5)──▶  group 成员集合
                                              │
...
这条推论也是本阶段与第五阶段最好的收口：**phase5 讲的是"边界怎么画"，phase11 讲的是
"边界里面装什么"**。
