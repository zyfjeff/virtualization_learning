...
> - 不需要先读第五阶段；**反过来，本阶段是第五阶段的地基**
>
> **建议阅读顺序：phase2 → phase11（本章）→ phase3 → phase5。**

---

## 📋 学习目标

前三阶段里，IOMMU 一直是以"结论"的形式出现的：phase3 说"直通设备的中断要经 IRTE"，
phase5 说"ACS 决定 group 边界，group 决定能否单独直通"。这些都对，但它们背后有一整层
...
- **PCIe/ACS/ATS 相关**：`pcie-base-spec-r6.0.pdf` 可用；VT-d 把部分语义（如 Device-TLB
  ...
  ACS/ATS 的判定语义已在第五阶段 §1.5 以源码为据核过，本阶段不重复，只做交叉链接。
...
core 怎么构造它。判定规则在 phase5 §1.4/§1.5，构造在本阶段。
→ [`group.md`](group.md)
...
- **第五阶段** [`../phase5-vfio/README.md`](../phase5-vfio/README.md)：group 判定（§1.4）、
  ACS/ATS（§1.5）、DMA 映射（§2）、MSI-X 直通（§3.3）。
- **第三阶段** [`../phase3-interrupts/README.md`](../phase3-interrupts/README.md)：IRTE、
