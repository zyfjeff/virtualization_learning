...
phase2 (EPT)           → 理解内存虚拟化
phase4 (中断)          → 理解中断路径
phase5 (vhost)         → 理解 vhost 加速
phase6 (VFIO)          → 理解设备直通
phase7 (时钟)          → 理解 TSC/kvmclock
phase8 (毕业建造)      → 综合运用
phase9 (性能优化)      → halt-polling/PLE 在 MicroVM 中的应用
phase10 (调试测试)     → bpftrace/perf 分析 MicroVM 性能
```
