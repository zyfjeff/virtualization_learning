...
> [`../phase11-debugging/`](../phase11-debugging/README.md)，本文一律不重复。
...
- 报告里写明顺序。`../phase9-capstone/practice/bench-halt-sweep.sh` 是固定升序扫
...
`../phase9-capstone/practice/bench-halt.sh` 开头记录 `ORIG=` 再恢复的写法。
...
`../phase9-capstone/practice/README.md` 项目4 M2 里，**单 vCPU** busy 15 s 就记到
...
2. **版本一致性**：
   ...
   - 要用 function tracer 时 `grep -w <fn> /sys/kernel/debug/tracing/available_filter_functions`
     ...
     （`parameters.md` §3、`../phase8-timer-virt/annotations.md:581`）。
...
| **PLE × 超卖** | 不超卖时 PLE 根本没有作用对象（无 CPU 可让），单 vCPU 测不出任何东西 —— `../phase9-capstone/practice/README.md` 项目4 已实测定性 | 必须有"1:1 绑核不超卖"的**阴性对照档**，见 `practice/bench-ple.md` |
...
| `perf kvm stat record -p $PID` | 只跟踪被包裹进程，**vCPU 线程退出全丢**。源码判据：`tools/perf/builtin-kvm.c:1959-1960` —— `if (target__none(&kvm->opts.target)) … system_wide = true;`，即**只有不给 target 时才自动 system-wide** | 实测结论见 `../phase9-capstone/practice/README.md` "测量陷阱备忘"；`../phase11-debugging/README.md` 场景 1 已按本文改为 `-a` |
...
| guest busybox ash 行缓冲 ≈1024 字节 | flood 类负载中途**静默停摆**，后续全部超时 | `../phase9-capstone/practice/README.md` 测量陷阱备忘（`bench-halt.sh` 限 800 字符） |
