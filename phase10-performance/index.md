...
| 出处 | `../phase9-capstone/practice/README.md` 项目4 M1（约 `:301-344`） |
| 数据 | `../phase9-capstone/practice/bench/boot-20260901-095559/boot.csv` |
...
| 关联 | 与 `../phase9-capstone/README.md` 的启动路径结论一致 |
...
| 出处 | `../phase9-capstone/practice/README.md` 项目4 M3（约 `:424-478`） |
...
| 出处 | `../phase9-capstone/practice/README.md` 项目4 M2（约 `:346-422`）、M4（`:474-478`） |
...
| 出处 | `../phase7-vfio/practice/README.md`（约 `:144-163`、`:342`、`:485`） |
...
| 出处 | `../phase8-timer-virt/practice/README.md` Experiment 3（`:142-264`，数字在 `:244-262`） |
...
| 出处 | `../phase6-virtio/practice/README.md`（约 `:55-98`），脚本 `vhost-perf-test.sh` |
...
| 主题 | 流传的说法 | 现状 | 待验证于 |
|---|---|---|---|
| Posted Interrupts 收益 | "延迟 <1 µs、吞吐提升 10–100 倍" | `../phase5-interrupts/posted-interrupts.md` 有机制详解（机制部分有规范支撑，见下 D 节勘误），但**收益倍数无任何实测** | `../phase5-interrupts/practice/ex4-pi-vs-remapped.sh` **已有脚本但结果未回写** → 优先补这个，不用新写 |
...
| 各时钟源读取成本 | "PIT 1000 ns / HPET 500 / APIC 300 / TSC 20 / kvmclock 30" | `../phase8-timer-virt/README.md:1076-1114`，**无实测** | 移交 `../phase8-timer-virt/practice/`（时钟基准归 phase7） |
| IOMMU strict/lazy 延迟 | 用 `dmar_perf_latency` 直方图量化 | `../phase3-iommu/practice/README.md` I.15：**方法已核实但无数据** | phase3 自己补 |
| `ptp_kvm` 对时精度 | "~1 µs" | `../phase8-timer-virt/README.md:1369`，无实测 | phase7 |
...
| **vCPU 迁移的代价** | `../phase5-interrupts/practice/ex6-vcpu-migration.sh` 只观测未落结论 | `practice/bench-migrate.md`（E3） |
...
1. 其它章节要引性能数字 → 写 `[<数字> 的实测见 index.md §1.x](../phase10-performance/index.md)`
